...
static struct platform_device *betelgeuse_gpios[] __initdata = {
	&alc5623_gpio,
};

static struct i2c_board_info __initdata betelgeuse_i2c_bus0_board_info[] = {
	{
...
static struct platform_device *betelgeuse_i2s_devices[] __initdata = {
	&tegra_i2s_device1,
	&tegra_i2s_device2,
	&spdif_dit_device,
	&tegra_spdif_device,
	...
	&tegra_pcm_device,
#endif
	&betelgeuse_audio_device, /* this must come last, as we need the DAS to be initialized to access the codec registers ! */
};
...
int __init betelgeuse_audio_register_devices(void)
{
	...
	ret = i2c_register_board_info(0, betelgeuse_i2c_bus0_board_info, 
		ARRAY_SIZE(betelgeuse_i2c_bus0_board_info)); 
	if (ret)
		return ret;
	
	ret = platform_add_devices(betelgeuse_gpios, ARRAY_SIZE(betelgeuse_gpios));
	if (ret)
		return ret;

	return platform_add_devices(betelgeuse_i2s_devices, ARRAY_SIZE(betelgeuse_i2s_devices));
}
