/*
 * arch/arm/mach-tegra/board-betelgeuse-bt.c
 *
...
static struct resource betelgeuse_bcm4329_rfkill_resources[] = {
	{
		.name   = "bcm4329_nshutdown_gpio",
		.start  = BETELGEUSE_BT_RST,
		.end    = BETELGEUSE_BT_RST,
		.flags  = IORESOURCE_IO,
	},
};

static struct platform_device betelgeuse_bcm4329_rfkill_device = {
	.name = "bcm4329_rfkill",
	.id             = -1,
	.num_resources  = ARRAY_SIZE(betelgeuse_bcm4329_rfkill_resources),
	.resource       = betelgeuse_bcm4329_rfkill_resources,
};

void __init betelgeuse_bt_rfkill(void)
{
	/*Add Clock Resource*/
	int res = clk_add_alias("bcm4329_32k_clk", betelgeuse_bcm4329_rfkill_device.name, \
				"blink", NULL);
	printk("Initializing BT RFKILL, clk_add_alias: %d", res);
	res = platform_device_register(&betelgeuse_bcm4329_rfkill_device);
	if (res)
		printk("Error on BT RFKILL reg, result: %d", res);
	return;
}

void __init betelgeuse_setup_bluesleep(void)
{
