...
static struct regulator_init_data ldo3_data = REGULATOR_INIT(ldo3, 1250, 3300, ON, NULL);
...
static void betelgeuse_board_suspend(int lp_state, enum suspend_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_SUSPEND_BEFORE_CPU))
		tegra_console_uart_suspend();
}

static void betelgeuse_board_resume(int lp_state, enum resume_stage stg)
{
...
static struct tegra_suspend_platform_data betelgeuse_suspend_data = {
	...
	.corereq_high	= false,
	.sysclkreq_high	= true,
	.board_suspend = betelgeuse_board_suspend,
	.board_resume = betelgeuse_board_resume,
};
...
static int __init betelgeuse_pcie_init(void)
{
	int ret;
...
late_initcall(betelgeuse_pcie_init);
